...
See [cli-and-dev.md](references/cli-and-dev.md#connic-test) for the test YAML schema.
...
- **Don't invent test assertions.** The only top-level assertion in `tests/*.yaml` is `expected_result` (a sandboxed expression with `output`, `error`, `status`, `context` bindings — `status` is `"completed"`, `"failed"`, `"cancelled"`, `"blocked"`, or `"awaiting_approval"`), plus `expected_tool_calls` and `expected_no_tool_calls`. There is no `expected_output_contains` or `expected_output_matches`.
